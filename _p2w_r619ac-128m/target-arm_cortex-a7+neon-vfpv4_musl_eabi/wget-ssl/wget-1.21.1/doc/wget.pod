...
downloaded file. Thus you may safely type B<wget -Q2m -i sites>---download
...
server appropriate directives (B<Cache-Control: no-cache> and
B<Pragma: no-cache>) to get the file from the remote service,
...
B<TLSv1_3> and B<PFS>.  If B<auto> is used, the SSL library is
...
Set the maximum number of subdirectories that Wget will recurse into to I<depth>.
...
Set B<-l 0> or B<-l inf> for infinite recursion depth.

	
	wget -r -l 0 http://<site>/1.html


Ideally, one would expect this to download just F<1.html>.
but unfortunately this is not the case, because B<-l 0> is equivalent to
B<-l inf>---that is, infinite recursion.  To download a single HTML
page (or a handful of them), specify them all on the command line and leave away B<-r>
and B<-l>. To download the essential items to view a single HTML page, see B<page requisites>.
...
Currently maintained by Darshit Shah E<lt>darnir@gnu.orgE<gt> and
Tim Rühsen E<lt>tim.ruehsen@gmx.deE<gt>.
