...
TF_BUILTIN(ProxyDeleteProperty, CodeStubAssembler) {
  ...
  if (block29.is_used()) {
    ca_.Bind(&block29);
    tmp9 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltin(Builtins::kDeleteProperty, parameter0, tmp6, parameter2, parameter3));
  ...
  if (block28.is_used()) {
    ...
    ca_.Branch(tmp12, &block30, std::vector<Node*>{}, &block31, std::vector<Node*>{});
  }
  ...
  if (block30.is_used()) {
    ...
    ca_.Branch(tmp14, &block32, std::vector<Node*>{}, &block33, std::vector<Node*>{});
  }
