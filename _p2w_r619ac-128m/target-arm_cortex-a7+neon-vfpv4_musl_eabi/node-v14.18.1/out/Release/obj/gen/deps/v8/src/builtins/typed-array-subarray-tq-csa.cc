...
USE(parameter1);
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    tmp2 = TypedArrayBuiltinsAssembler(state_).GetBuffer(TNode<Context>{parameter0}, TNode<JSTypedArray>{tmp0});
  ...
  if (block11.is_used()) {
    ...
    ca_.Branch(tmp26, &block16, std::vector<Node*>{}, &block17, std::vector<Node*>{});
  }
  ...
  if (block17.is_used()) {
    ...
    ca_.Branch(tmp29, &block18, std::vector<Node*>{}, &block19, std::vector<Node*>{});
  }
