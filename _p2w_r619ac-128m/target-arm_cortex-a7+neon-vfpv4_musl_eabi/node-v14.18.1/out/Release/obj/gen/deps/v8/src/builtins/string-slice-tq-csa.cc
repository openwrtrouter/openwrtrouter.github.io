...
USE(parameter1);
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = FromConstexpr_String_constexpr_string_0(state_, "String.prototype.slice");
  ...
  if (block7.is_used()) {
    ...
    tmp14 = CodeStubAssembler(state_).UintPtrLessThanOrEqual(TNode<UintPtrT>{phi_bb7_10}, TNode<UintPtrT>{phi_bb3_8});
  ...
  if (block10.is_used()) {
    ca_.Bind(&block10);
    tmp16 = StringBuiltinsAssembler(state_).SubString(TNode<String>{tmp1}, TNode<UintPtrT>{phi_bb3_8}, TNode<UintPtrT>{phi_bb7_10});
