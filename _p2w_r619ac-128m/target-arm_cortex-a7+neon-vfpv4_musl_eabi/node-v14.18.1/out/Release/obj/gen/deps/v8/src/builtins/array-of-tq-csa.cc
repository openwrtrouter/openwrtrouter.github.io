...
USE(parameter1);
  ...
  if (block4.is_used()) {
    ca_.Bind(&block4);
    tmp3 = CodeStubAssembler(state_).ArrayCreate(TNode<Context>{parameter0}, TNode<Number>{tmp0});
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    tmp4 = CodeStubAssembler(state_).Construct(TNode<Context>{parameter0}, TNode<JSReceiver>{tmp1}, TNode<Object>{tmp0});
    ca_.Goto(&block1, tmp4);
  }

  TNode<JSReceiver> phi_bb1_10;
  TNode<Smi> tmp5;
  if (block1.is_used()) {
    ca_.Bind(&block1, &phi_bb1_10);
  ...
  if (block7.is_used()) {
    ...
    tmp6 = CodeStubAssembler(state_).SmiLessThan(TNode<Smi>{phi_bb7_11}, TNode<Smi>{tmp0});
    ca_.Branch(tmp6, &block5, std::vector<Node*>{phi_bb7_11}, &block6, std::vector<Node*>{phi_bb7_11});
  }
  ...
  if (block5.is_used()) {
    ...
    tmp7 = Convert_intptr_Smi_0(state_, TNode<Smi>{phi_bb5_11});
    ...
    tmp9 = CodeStubAssembler(state_).CallBuiltin(Builtins::kFastCreateDataProperty, parameter0, phi_bb1_10, phi_bb5_11, tmp8);
    tmp10 = FromConstexpr_Smi_constexpr_int31_0(state_, 1);
    tmp11 = CodeStubAssembler(state_).SmiAdd(TNode<Smi>{phi_bb5_11}, TNode<Smi>{tmp10});
    ca_.Goto(&block7, tmp11);
  }

  TNode<Smi> phi_bb6_11;
  if (block6.is_used()) {
    ca_.Bind(&block6, &phi_bb6_11);
    CodeStubAssembler(state_).SetPropertyLength(TNode<Context>{parameter0}, TNode<Object>{phi_bb1_10}, TNode<Number>{tmp0});
    arguments.PopAndReturn(phi_bb1_10);
  }
}
