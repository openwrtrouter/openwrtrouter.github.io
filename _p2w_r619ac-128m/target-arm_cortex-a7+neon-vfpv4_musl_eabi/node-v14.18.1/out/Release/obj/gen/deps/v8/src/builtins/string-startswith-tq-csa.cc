...
USE(parameter1);
  ...
  if (block5.is_used()) {
    ...
    ca_.Branch(tmp15, &block7, std::vector<Node*>{}, &block8, std::vector<Node*>{});
  }
  ...
  if (block8.is_used()) {
    ...
    tmp17 = TryFastStringCompareSequence_0(state_, TNode<String>{tmp5}, TNode<String>{tmp7}, TNode<UintPtrT>{phi_bb5_10}, TNode<UintPtrT>{tmp13}, &label18);
    ca_.Goto(&block11);
    if (label18.is_used()) {
      ca_.Bind(&label18);
      ca_.Goto(&block12);
  ...
  if (block12.is_used()) {
    ca_.Bind(&block12);
    tmp19 = Convert_Number_uintptr_0(state_, TNode<UintPtrT>{phi_bb5_10});
    tmp20 = TORQUE_CAST(CodeStubAssembler(state_).CallRuntime(Runtime::kStringCompareSequence, parameter0, tmp5, tmp7, tmp19)); 
