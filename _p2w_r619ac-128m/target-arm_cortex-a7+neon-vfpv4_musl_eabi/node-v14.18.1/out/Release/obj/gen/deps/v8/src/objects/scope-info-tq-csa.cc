...
TNode<Uint32T> LoadScopeInfoFlags_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<ScopeInfo> p_scopeInfo) {
  ...
  if (block0.is_used()) {
    ...
    tmp3 = CodeStubAssembler(state_).LoadReference<Smi>(CodeStubAssembler::Reference{p_scopeInfo, tmp2});
    tmp4 = Convert_intptr_Smi_0(state_, TNode<Smi>{tmp3});
    tmp5 = FromConstexpr_intptr_constexpr_int32_0(state_, ScopeInfo::Fields::kFlags);
  ...
  if (block6.is_used()) {
    ...
    std::tie(tmp12, tmp13) = NewReference_Object_0(state_, TNode<HeapObject>{p_scopeInfo}, TNode<IntPtrT>{tmp11}).Flatten();
    ...
    tmp16 = Convert_ScopeFlags_Smi_0(state_, TNode<Smi>{tmp15});
    ca_.Goto(&block9);
  }
