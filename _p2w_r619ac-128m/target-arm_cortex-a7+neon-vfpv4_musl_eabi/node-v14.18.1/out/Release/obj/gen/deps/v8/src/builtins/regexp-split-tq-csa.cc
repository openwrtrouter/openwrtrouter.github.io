...
TF_BUILTIN(RegExpSplit, CodeStubAssembler) {
  ...
  if (block3.is_used()) {
    ...
    ca_.Branch(tmp7, &block7, std::vector<Node*>{phi_bb3_4}, &block8, std::vector<Node*>{phi_bb3_4});
  }
...
USE(parameter1);
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    tmp9 = CodeStubAssembler(state_).CallBuiltin(Builtins::kRegExpSplit, parameter0, tmp6, tmp3, tmp5);
