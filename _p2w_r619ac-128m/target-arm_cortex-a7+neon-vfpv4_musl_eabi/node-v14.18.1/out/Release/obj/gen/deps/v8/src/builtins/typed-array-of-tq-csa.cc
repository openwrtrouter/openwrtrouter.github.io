...
USE(parameter1);
  ...
  if (block9.is_used()) {
    ...
    ca_.Branch(tmp10, &block7, std::vector<Node*>{phi_bb9_11}, &block8, std::vector<Node*>{phi_bb9_11});
  }
...
tmp13 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(Builtins::CallableFor(ca_.isolate(),ExampleBuiltinForTorqueFunctionPointerType(3)).descriptor(), tmp8, parameter0, tmp4, phi_bb7_11, tmp12));
    ...
    ca_.Branch(tmp15, &block12, std::vector<Node*>{phi_bb7_11, phi_bb7_11, phi_bb7_11}, &block13, std::vector<Node*>{phi_bb7_11, phi_bb7_11, phi_bb7_11});
  }
