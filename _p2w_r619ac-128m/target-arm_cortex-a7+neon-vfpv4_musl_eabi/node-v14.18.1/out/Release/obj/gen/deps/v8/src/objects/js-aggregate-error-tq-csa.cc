...
TF_BUILTIN(AggregateErrorConstructor, CodeStubAssembler) {
  ...
  USE(parameter0);
  TNode<JSFunction> parameter1 = UncheckedCast<JSFunction>(Parameter(Descriptor::kJSTarget));
USE(parameter1);
...
USE(parameter2);
  ...
  if (block0.is_used()) {
    ...
    tmp2 = TORQUE_CAST(CodeStubAssembler(state_).CallRuntime(Runtime::kConstructAggregateErrorHelper, parameter0, parameter1, parameter2, tmp1)); 
    ...
    tmp5 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltin(Builtins::kIterableToFixedArrayWithSymbolLookupSlow, parameter0, tmp4));
    CodeStubAssembler(state_).MakeFixedArrayCOW(TNode<FixedArray>{tmp5});
    tmp6 = FromConstexpr_intptr_constexpr_int31_0(state_, 12);
    CodeStubAssembler(state_).StoreReference<HeapObject>(CodeStubAssembler::Reference{tmp2, tmp6}, tmp5);
    arguments.PopAndReturn(tmp2);
  }
}

TF_BUILTIN(AggregateErrorPrototypeErrorsGetter, CodeStubAssembler) {
  compiler::CodeAssemblerState* state_ = state();  compiler::CodeAssembler ca_(state());
  TNode<NativeContext> parameter0 = UncheckedCast<NativeContext>(Parameter(Descriptor::kContext));
  USE(parameter0);
...
USE(parameter1);
  ...
  if (block0.is_used()) {
    ...
    tmp0 = Cast_JSAggregateError_1(state_, TNode<Context>{parameter0}, TNode<Object>{parameter1}, &label1);
    ca_.Goto(&block3);
    if (label1.is_used()) {
      ca_.Bind(&label1);
      ca_.Goto(&block4);
  ...
  if (block4.is_used()) {
    ca_.Bind(&block4);
    tmp2 = FromConstexpr_Object_constexpr_string_0(state_, "JSAggregateError.prototype.errors.get");
    tmp3 = FromConstexpr_Object_constexpr_string_0(state_, "AggregateError");
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, MessageTemplate::kNotGeneric, TNode<Object>{tmp2}, TNode<Object>{tmp3});
  }
  ...
  if (block3.is_used()) {
    ...
    tmp5 = CodeStubAssembler(state_).LoadReference<HeapObject>(CodeStubAssembler::Reference{tmp0, tmp4});
    tmp6 = UnsafeCast_FixedArray_0(state_, TNode<Context>{parameter0}, TNode<Object>{tmp5});
    tmp7 = CreateJSArrayWithElements_0(state_, TNode<Context>{parameter0}, TNode<FixedArray>{tmp6});
...
TNode<JSAggregateError> Cast_JSAggregateError_1(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<Object> p_o, compiler::CodeAssemblerLabel* label_CastError) {
  ...
  if (block3.is_used()) {
    ...
    tmp2 = Cast_JSAggregateError_0(state_, TNode<HeapObject>{tmp0}, &label3);
    ca_.Goto(&block5);
    if (label3.is_used()) {
      ca_.Bind(&label3);
      ca_.Goto(&block6);
  ...
  if (block1.is_used()) {
    ...
    ca_.Bind(&block7);
  return TNode<JSAggregateError>{tmp2};
}
