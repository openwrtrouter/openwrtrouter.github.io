...
TNode<CoverageInfo> GetCoverageInfo_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSFunction> p_function, compiler::CodeAssemblerLabel* label_IfNoCoverageInfo) {
  ...
  if (block0.is_used()) {
    ...
    tmp4 = Cast_DebugInfo_0(state_, TNode<Context>{p_context}, TNode<HeapObject>{tmp3}, &label5);
    ca_.Goto(&block5);
    if (label5.is_used()) {
      ca_.Bind(&label5);
      ca_.Goto(&block6);
...
void IncrementBlockCount_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<CoverageInfo> p_coverageInfo, TNode<Smi> p_slot) {
  ...
  if (block0.is_used()) {
    ...
    tmp3 = CodeStubAssembler(state_).LoadReference<Int32T>(CodeStubAssembler::Reference{p_coverageInfo, tmp2});
    tmp4 = Convert_intptr_int32_0(state_, TNode<Int32T>{tmp3});
    tmp5 = Convert_intptr_Smi_0(state_, TNode<Smi>{p_slot});
  ...
  if (block10.is_used()) {
    ...
    std::tie(tmp12, tmp13) = NewReference_CoverageInfoSlot_0(state_, TNode<HeapObject>{p_coverageInfo}, TNode<IntPtrT>{tmp11}).Flatten();
    ...
    tmp16 = CodeStubAssembler(state_).LoadReference<Int32T>(CodeStubAssembler::Reference{tmp12, tmp15});
    tmp17 = FromConstexpr_int32_constexpr_int31_0(state_, 1);
    tmp18 = CodeStubAssembler(state_).Int32Add(TNode<Int32T>{tmp16}, TNode<Int32T>{tmp17});
    CodeStubAssembler(state_).StoreReference<Int32T>(CodeStubAssembler::Reference{tmp12, tmp15}, tmp18);
    ca_.Goto(&block13);
  }
