...
TF_BUILTIN(ProxyHasProperty, CodeStubAssembler) {
  ...
  if (block33.is_used()) {
    ca_.Bind(&block33);
   CodeStubAssembler(state_).TailCallBuiltin(Builtins::kHasProperty, parameter0, tmp6, parameter2);
  }
  ...
  if (block32.is_used()) {
    ...
    ca_.Branch(tmp11, &block34, std::vector<Node*>{}, &block35, std::vector<Node*>{});
  }
