...
TF_BUILTIN(GetIteratorWithFeedback, CodeStubAssembler) {
  ...
  USE(parameter2);
  TNode<TaggedIndex> parameter3 = UncheckedCast<TaggedIndex>(Parameter(Descriptor::ParameterIndex<2>()));
  USE(parameter3);
  ...
  if (block4.is_used()) {
    ...
    tmp3 = CodeStubAssembler(state_).CallBuiltin(Builtins::kLoadIC, parameter0, parameter1, tmp2, parameter2, ca_.UncheckedCast<FeedbackVector>(parameter4));
  ...
  if (block1.is_used()) {
    ...
    tmp7 = CodeStubAssembler(state_).CallBuiltin(Builtins::kCallIteratorWithFeedback, parameter0, parameter1, phi_bb1_5, tmp6, parameter4);
...
TF_BUILTIN(CallIteratorWithFeedback, CodeStubAssembler) {
  ...
  if (block0.is_used()) {
    ...
    CollectCallFeedback_1(state_, TNode<Object>{parameter2}, TNode<Context>{parameter0}, TNode<HeapObject>{parameter4}, TNode<UintPtrT>{tmp1});
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    tmp4 = CodeStubAssembler(state_).Call(TNode<Context>{parameter0}, TNode<Object>{tmp2}, TNode<Object>{parameter1});
...
void IteratorCloseOnException_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TorqueStructIteratorRecord p_iterator) {
  ...
  if (block10.is_used()) {
    ...
    ca_.Branch(phi_bb10_5, &block5, std::vector<Node*>{}, &block6, std::vector<Node*>{});
  }
