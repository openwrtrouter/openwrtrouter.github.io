...
TNode<JSStringIterator> NewJSStringIterator_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<String> p_string, TNode<Smi> p_nextIndex) {
  ...
  if (block0.is_used()) {
    ...
    CodeStubAssembler(state_).StoreReference<String>(CodeStubAssembler::Reference{tmp4, tmp8}, p_string);
    tmp9 = FromConstexpr_intptr_constexpr_int31_0(state_, 16);
    CodeStubAssembler(state_).StoreReference<Smi>(CodeStubAssembler::Reference{tmp4, tmp9}, p_nextIndex);
...
USE(parameter1);
  ...
  if (block3.is_used()) {
    ...
    ca_.Branch(tmp9, &block5, std::vector<Node*>{}, &block6, std::vector<Node*>{});
  }
