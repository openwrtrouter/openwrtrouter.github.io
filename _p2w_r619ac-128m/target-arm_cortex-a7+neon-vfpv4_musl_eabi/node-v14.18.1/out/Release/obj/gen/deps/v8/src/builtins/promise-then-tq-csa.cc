...
TNode<BoolT> IsPromiseSpeciesLookupChainIntact_0(compiler::CodeAssemblerState* state_, TNode<NativeContext> p_nativeContext, TNode<Map> p_promiseMap) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = FromConstexpr_NativeContextSlot_constexpr_PROMISE_PROTOTYPE_INDEX_0(state_, Context::Field::PROMISE_PROTOTYPE_INDEX);
  ...
  if (block3.is_used()) {
    ...
    tmp5 = CodeStubAssembler(state_).LoadReference<HeapObject>(CodeStubAssembler::Reference{p_promiseMap, tmp4});
    tmp6 = CodeStubAssembler(state_).TaggedNotEqual(TNode<HeapObject>{tmp5}, TNode<Object>{tmp1});
  ...
  if (block5.is_used()) {
    ca_.Bind(&block5);
    tmp8 = CodeStubAssembler(state_).IsPromiseSpeciesProtectorCellInvalid();
...
USE(parameter1);
  ...
  if (block3.is_used()) {
    ...
    tmp4 = CodeStubAssembler(state_).LoadContextElement(TNode<NativeContext>{parameter0}, TNode<IntPtrT>{tmp3});
    tmp5 = UnsafeCast_JSFunction_0(state_, TNode<Context>{parameter0}, TNode<Object>{tmp4});
    ...
    tmp8 = IsPromiseSpeciesLookupChainIntact_0(state_, TNode<NativeContext>{parameter0}, TNode<Map>{tmp7});
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    tmp9 = CodeStubAssembler(state_).SpeciesConstructor(TNode<Context>{parameter0}, TNode<Object>{tmp0}, TNode<JSReceiver>{tmp5});
    tmp10 = CodeStubAssembler(state_).TaggedEqual(TNode<HeapObject>{tmp9}, TNode<HeapObject>{tmp5});
    ca_.Branch(tmp10, &block9, std::vector<Node*>{}, &block10, std::vector<Node*>{});
  }
  ...
  if (block10.is_used()) {
    ca_.Bind(&block10);
    tmp11 = True_0(state_);
    tmp12 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltin(Builtins::kNewPromiseCapability, parameter0, tmp9, tmp11));
    ...
    ca_.Goto(&block5, tmp12, tmp14);
  }

  TNode<JSPromise> tmp15;
  if (block6.is_used()) {
    ca_.Bind(&block6);
    tmp15 = NewJSPromise_0(state_, TNode<Context>{parameter0}, TNode<Object>{tmp0});
    ca_.Goto(&block5, tmp15, tmp15);
  }
  ...
  if (block5.is_used()) {
    ...
    tmp17 = CastOrDefault_Callable_JSAny_Undefined_0(state_, TNode<Context>{parameter0}, TNode<Object>{parameter2}, TNode<Oddball>{tmp16});
    tmp18 = Undefined_0(state_);
    tmp19 = CastOrDefault_Callable_JSAny_Undefined_0(state_, TNode<Context>{parameter0}, TNode<Object>{parameter3}, TNode<Oddball>{tmp18});
    PerformPromiseThenImpl_0(state_, TNode<Context>{parameter0}, TNode<JSPromise>{tmp0}, TNode<HeapObject>{tmp17}, TNode<HeapObject>{tmp19}, TNode<HeapObject>{phi_bb5_6});
