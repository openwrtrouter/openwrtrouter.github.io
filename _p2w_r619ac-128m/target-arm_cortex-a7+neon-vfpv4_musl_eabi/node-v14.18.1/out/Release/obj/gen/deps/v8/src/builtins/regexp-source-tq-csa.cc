...
USE(parameter1);
  ...
  if (block6.is_used()) {
    ca_.Bind(&block6);
    tmp6 = CodeStubAssembler(state_).SmiConstant(v8::Isolate::kRegExpPrototypeSourceGetter);
    CodeStubAssembler(state_).CallRuntime(Runtime::kIncrementUseCounter, parameter0, tmp6);
