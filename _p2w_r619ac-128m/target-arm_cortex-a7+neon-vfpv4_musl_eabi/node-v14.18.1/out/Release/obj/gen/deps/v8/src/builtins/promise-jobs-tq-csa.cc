...
TF_BUILTIN(PromiseResolveThenableJob, CodeStubAssembler) {
  ...
  if (block0.is_used()) {
    ...
    tmp1 = FromConstexpr_NativeContextSlot_constexpr_PROMISE_THEN_INDEX_0(state_, Context::Field::PROMISE_THEN_INDEX);
    ...
    tmp4 = CodeStubAssembler(state_).LoadReference<Map>(CodeStubAssembler::Reference{parameter2, tmp3});
    tmp5 = CodeStubAssembler(state_).TaggedEqual(TNode<MaybeObject>{parameter3}, TNode<MaybeObject>{tmp2});
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    tmp6 = CodeStubAssembler(state_).IsJSPromiseMap(TNode<Map>{tmp4});
  ...
  if (block5.is_used()) {
    ...
    ca_.Branch(phi_bb5_8, &block6, std::vector<Node*>{}, &block7, std::vector<Node*>{});
  }
  ...
  if (block6.is_used()) {
    ca_.Bind(&block6);
    tmp8 = CodeStubAssembler(state_).NeedsAnyPromiseHooks();
  ...
  if (block7.is_used()) {
    ...
    ca_.Goto(&block8, tmp10);
  }

  TNode<BoolT> phi_bb8_8;
  if (block8.is_used()) {
    ca_.Bind(&block8, &phi_bb8_8);
    ca_.Branch(phi_bb8_8, &block9, std::vector<Node*>{}, &block10, std::vector<Node*>{});
  }

  TNode<BoolT> tmp11;
  if (block9.is_used()) {
    ca_.Bind(&block9);
    tmp11 = IsPromiseSpeciesLookupChainIntact_0(state_, TNode<NativeContext>{tmp0}, TNode<Map>{tmp4});
  ...
  if (block11.is_used()) {
    ...
    ca_.Branch(phi_bb11_8, &block1, std::vector<Node*>{}, &block2, std::vector<Node*>{});
  }
  ...
  if (block1.is_used()) {
    ca_.Bind(&block1);
    tmp13 = UnsafeCast_JSPromise_0(state_, TNode<Context>{parameter0}, TNode<Object>{parameter2});
    tmp14 = CodeStubAssembler(state_).UndefinedConstant();
    tmp15 = CodeStubAssembler(state_).UndefinedConstant();
    tmp16 = CodeStubAssembler(state_).CallBuiltin(Builtins::kPerformPromiseThen, parameter0, tmp13, tmp14, tmp15, parameter1);
  ...
  if (block2.is_used()) {
    ...
    std::tie(tmp18, tmp19) = CreatePromiseResolvingFunctions_0(state_, TNode<Context>{parameter0}, TNode<JSPromise>{parameter1}, TNode<Object>{tmp17}, TNode<NativeContext>{tmp0}).Flatten();
    ...
    tmp23 = CodeStubAssembler(state_).Call(TNode<Context>{parameter0}, TNode<Object>{tmp20}, TNode<Object>{parameter2}, TNode<Object>{tmp18}, TNode<Object>{tmp19});
    }
  ...
  if (block15.is_used()) {
    ca_.Bind(&block15);
    ca_.Goto(&block14, tmp22);
  }

  if (block16.is_used()) {
    ca_.Bind(&block16);
    ca_.Goto(&block14, tmp25);
  }
  ...
  if (block14.is_used()) {
    ...
    tmp26 = UnsafeCast_Callable_0(state_, TNode<Context>{parameter0}, TNode<Object>{tmp19});
    tmp27 = Undefined_0(state_);
    tmp28 = CodeStubAssembler(state_).Call(TNode<Context>{parameter0}, TNode<Object>{tmp26}, TNode<Object>{tmp27}, TNode<Object>{phi_bb14_11});
