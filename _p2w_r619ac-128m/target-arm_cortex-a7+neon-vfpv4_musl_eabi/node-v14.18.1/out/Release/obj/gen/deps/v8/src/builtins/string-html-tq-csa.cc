...
TF_BUILTIN(CreateHTML, CodeStubAssembler) {
  ...
  if (block0.is_used()) {
    ...
    ca_.Branch(tmp4, &block1, std::vector<Node*>{}, &block2, std::vector<Node*>{tmp2});
  }
...
USE(parameter1);
  ...
  if (block0.is_used()) {
    ...
    tmp5 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltin(Builtins::kCreateHTML, parameter0, parameter1, tmp2, tmp3, tmp4, tmp1));
...
USE(parameter1);
  ...
  if (block0.is_used()) {
    ...
    tmp4 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltin(Builtins::kCreateHTML, parameter0, parameter1, tmp2, tmp3, tmp0, tmp1));
