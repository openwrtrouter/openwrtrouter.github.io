...
TorqueStructFrameWithArgumentsInfo_0 GetFrameWithArgumentsInfo_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context) {
  ...
  compiler::CodeAssemblerParameterizedLabel<RawPtrT, BInt> block1(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  ...
  if (block4.is_used()) {
    ...
    tmp10 = LoadCallerFromFrame_0(state_, TNode<RawPtrT>{tmp1});
    compiler::CodeAssemblerLabel label12(&ca_);
    tmp11 = Cast_ArgumentsAdaptorFrame_0(state_, TNode<Context>{p_context}, TNode<RawPtrT>{tmp10}, &label12);
    ca_.Goto(&block8);
    if (label12.is_used()) {
      ca_.Bind(&label12);
      ca_.Goto(&block9);
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    tmp13 = LoadLengthFromAdapterFrame_0(state_, TNode<Context>{p_context}, TNode<RawPtrT>{tmp11});
    tmp14 = Convert_bint_Smi_0(state_, TNode<Smi>{tmp13});
    ca_.Goto(&block1, tmp11, tmp14);
  }

  TNode<RawPtrT> phi_bb1_1;
  TNode<BInt> phi_bb1_2;
  if (block1.is_used()) {
    ca_.Bind(&block1, &phi_bb1_1, &phi_bb1_2);
    ca_.Goto(&block10);
  }

    ca_.Bind(&block10);
  return TorqueStructFrameWithArgumentsInfo_0{TNode<RawPtrT>{phi_bb1_1}, TNode<BInt>{phi_bb1_2}, TNode<BInt>{tmp9}};
}
