...
template <class D, class P>
class TorqueGeneratedJSAggregateError : public P {
  static_assert(std::is_same<JSAggregateError, D>::value,
    "Use this class as direct base for JSAggregateError.");
  static_assert(std::is_same<JSObject, P>::value,
    "Pass in JSObject as second template parameter for TorqueGeneratedJSAggregateError.");
 public: 
  using Super = P;

  // Torque type: (Undefined | class FixedArray)
  inline HeapObject errors() const;
  inline HeapObject errors(const Isolate* isolates) const;
  inline void set_errors(HeapObject value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
...
#ifdef VERIFY_HEAP
  V8_EXPORT_PRIVATE void JSAggregateErrorVerify(Isolate* isolate);
#endif  // VERIFY_HEAP

  static constexpr int kStartOfStrongFieldsOffset = P::kHeaderSize;
  static constexpr int kErrorsOffset = P::kHeaderSize;
  static constexpr int kErrorsOffsetEnd = kErrorsOffset + kTaggedSize - 1;
  static constexpr int kEndOfStrongFieldsOffset = kErrorsOffsetEnd + 1;
  static constexpr int kStartOfWeakFieldsOffset = kErrorsOffsetEnd + 1;
  static constexpr int kEndOfWeakFieldsOffset = kErrorsOffsetEnd + 1;
  static constexpr int kHeaderSize = kErrorsOffsetEnd + 1;

  friend class Factory;

 public:
  template <class DAlias = D>
  constexpr TorqueGeneratedJSAggregateError() : P() {
    static_assert(std::is_base_of<TorqueGeneratedJSAggregateError, 
      DAlias>::value,
      "class TorqueGeneratedJSAggregateError should be used as direct base for JSAggregateError.");
  }
protected:
  inline explicit TorqueGeneratedJSAggregateError(Address ptr);
  // Special-purpose constructor for subclasses that have fast paths where
  // their ptr() is a Smi.
  inline explicit TorqueGeneratedJSAggregateError(Address ptr, HeapObject::AllowInlineSmiStorage allow_smi);
};
...
class TorqueGeneratedSourceTextModule : public P {
 ...
 public: 
  ...
  inline HeapObject cycle_root(const Isolate* isolates) const;
  ...
  V8_INLINE static constexpr int32_t SizeFor() {
    DCHECK(kHeaderSize == kSize && kHeaderSize == 76);
    int32_t size = kHeaderSize;
    return size;
  }
